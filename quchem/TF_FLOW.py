...
def Gradient_funct_NORMAL(theta_guess_list, HF_initial_state):
    ...
    for j in range(len(theta_guess_list)):
        ...
        partial_gradient_list.append((Gradient, theta)) #append(Gradient)  #append((Gradient, theta))
...
Energy_Calc_tensor = Calc_Energy_TENSOR(theta_list_TENSOR, HF_initial_state, num_shots)
...
_, variables = zip(*optimizer.compute_gradients(Energy_Calc_tensor))

d_theta1_d_theta = tf.placeholder(tf.float32, shape=(), name='d_theta1_d_theta')
d_theta2_d_theta = tf.placeholder(tf.float32, shape=(), name='d_theta2_d_theta')
d_theta3_d_theta = tf.placeholder(tf.float32, shape=(), name='d_theta3_d_theta')

place_holder_list = [d_theta1_d_theta, d_theta2_d_theta, d_theta3_d_theta]
...
def optimize():
    with tf.Session() as session:
        ...
        print("starting at Angles:", Angles, "Energy:", Calc_Energy_NORMAL(Angles, HF_initial_state, num_shots=num_shots))
        for step in range(max_iter):
            ...
            if step == 0:
                ...
                Angles = [session.run(theta_tensor) for theta_tensor in theta_list_TENSOR]
                grads_and_vars = Gradient_funct_NORMAL(Angles, HF_initial_state)

                grad_sub_dict = {d_theta1_d_theta: grads_and_vars[0][0], d_theta2_d_theta: grads_and_vars[1][0],
                                 d_theta3_d_theta: grads_and_vars[2][0]}  # {placeholder: value}

                train_step.run(feed_dict=grad_sub_dict)

                NEW_Angles = [session.run(theta_tensor) for theta_tensor in theta_list_TENSOR]
                print("step", step, "Angles:", NEW_Angles, "Energy:",
                      Calc_Energy_NORMAL(NEW_Angles, HF_initial_state, num_shots=num_shots))
            else:
                Angles = [session.run(theta_tensor) for theta_tensor in theta_list_TENSOR]
                grads_and_vars = Gradient_funct_NORMAL(Angles, HF_initial_state)
            ...
            #
            # NEW_Angles = [session.run(theta_tensor) for theta_tensor in theta_list_TENSOR]
            # print("step", step, "Angles:", NEW_Angles, "Energy:", Calc_Energy_NORMAL(NEW_Angles, HF_initial_state, num_shots=num_shots))
