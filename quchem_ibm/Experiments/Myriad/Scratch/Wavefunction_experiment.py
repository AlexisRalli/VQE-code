...
def main(N_repeats):
    ...
    file_path_to_LCU_data='string'
    file_path_to_STANDARD_data='string'
    file_path_to_seq_rot_data = 'string'
...
# def main():
#
#     file_path_to_seq_rot_data = '/home/ucapar1/Scratch/LiH_wave_exp/molecule=LiH___n_shots=1___method=seq_rot_VQE___time=2020Sep28-215957459556.pickle'
#
#     with open(file_path_to_seq_rot_data, 'rb') as handle:
#         output_data_seq_rot = pickle.load(handle)
#
...
#     for index, exp_dict in tqdm(enumerate(output_data_seq_rot['experiment_dict']), ascii=True, desc='Getting_State'):
#         state = output_data_seq_rot['state_vector_list'][index].data
#
#         if 'Ps' in exp_dict.keys():
#             Ps = exp_dict['Ps']
#             Ps_Qubit_list, Ps_str_list = zip(*(list(*Ps.terms.keys())))
#
#             gamma_l = exp_dict['gamma_l']
#
#             simulated_output_dict[index] = {
#                                                 'Ps_Qubit_list': Ps_Qubit_list,
#                                                 'Ps_str_list':Ps_str_list,
#                                                 'gamma_l':gamma_l,
#                                                 'wavefunction': state.tolist()
#                                             }
#         else:
#             qubitOp = exp_dict['qubitOp']
#             coeff = exp_dict['coeff']
#
#             qubitOp_Qubit_list, qubitOp_str_list = zip(*(list(*qubitOp.terms.keys())))
#
#             simulated_output_dict[index] = {
#                                                 'qubitOp_Qubit_list': qubitOp_Qubit_list,
#                                                  'qubitOp_str_list':qubitOp_str_list,
#                                                 'coeff':coeff,
#                                                  'wavefunction': state.tolist()
#                                             }
#
#     output['simulated_wavefunction']=simulated_output_dict
#
#     with open('SEQ_ROT.json', 'w') as f:
#         json.dump(output, f)
#
#     return output
#
...
def main():
    ...
    simulated_output_dict={}
    for index, exp_dict in tqdm(enumerate(output_data_seq_rot['experiment_dict']), ascii=True, desc='Getting_State'):
        state =output_data_seq_rot['state_vector_list'][index]
        SIMPLIFIED_STATE = Statevector(np.around(state.data, decimals=12))

        if SIMPLIFIED_STATE.data.shape[0]!=4096:
            raise ValueError('qubit state is incorrect!')
    ...
    with open('SEQ_ROT.pickle', 'wb') as fhandle:
            pickle.dump(output, fhandle, protocol=pickle.HIGHEST_PROTOCOL)
...
def main():
    ...
    for index, exp_dict in tqdm(enumerate(output_data_seq_rot['experiment_dict']), ascii=True, desc='Getting_State'):
        ...
        if SIMPLIFIED_STATE.data.shape[0]!=4096:
            print('qubit state is incorrect! index is {} and shape = {}'.format(index, SIMPLIFIED_STATE.data.shape[0]))

        print(output_data_seq_rot['experiment_dict3'][index])
   #      if 'Ps' in exp_dict.keys():
   #          Ps = exp_dict['Ps']
   #          #Ps_Qubit_list, Ps_str_list = zip(*(list(*Ps.terms.keys())))
   #
