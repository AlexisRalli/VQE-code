...
#     _, _, _, ham_out, ham_noncon_out = c.diagonalize_G(model,fn_form,ep_state, ham, ham_noncon)
    ...
    true_gs= csvqe_results[key][0]


    #### NEW LCU implementation of CS VQE (R_LCU performed classically)
    _,_,_, order =c_LCU.csvqe_approximations_heuristic_LCU(ham,
                                       ham_noncon,
                                       n_qubits, 
                                       true_gs, 
                                       N_index, 
    ...
    reduced_Con_hamiltonians = c_LCU.get_reduced_hamiltonians_LCU(ham, # Con_H,
                               model,
                               fn_form,
                               ep_state,
                               order, 
...
file_name1 = 'NEW_METHOD_LCU_CS_VQE_experimental_ordering_anticommuting_vs_standard_conH__{}.pickle'.format(unique_file_time)
...
file_name2 = 'NEW_METHOD_LCU_CS_VQE_experimental_ordering_updated_hamiltonians__{}.pickle'.format(unique_file_time)
...
### save LCU energies!
file_name_LCU = 'NEW_METHOD_LCU_CS_VQE_E_LCU_all_EXP__{}.pickle'.format(unique_file_time)
