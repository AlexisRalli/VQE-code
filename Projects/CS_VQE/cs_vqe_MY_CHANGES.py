...
def diagonalize_epistemic(model,fn_form,ep_state, N_Qubits, check_reduction=False):
...
def quantum_correction(ham,model,fn_form,ep_state):
    ...
    R_SeqRot, rotations, diagonal_set, vals = diagonalize_epistemic(model,fn_form,ep_state)
...
def get_reduced_hamiltonians(ham,model,fn_form,ep_state,order, N_Qubits, check_reduction=False):

    R_SeqRot, rotations, diagonal_set, vals = diagonalize_epistemic(model,fn_form,ep_state, N_Qubits, check_reduction=check_reduction)
    ...
    for k in range(order_len+1):
        ...
        if R_SeqRot is not None:
            ...
            ham_rotated = conv_scr.Openfermion_to_dict(rot_H, n_q)
        #     openF_R_SeqRot = reduce(lambda x,y: x+y, R_SeqRot)
            
        #     rotated_H = openF_R_SeqRot*Ham_openF*hermitian_conjugated(openF_R_SeqRot) # R H R_dagger!
        #     post_SeqRot_rot_ham = conv_scr.Openfermion_to_dict(rotated_H, n_q)
...
def contextual_subspace_approximations(ham,model,fn_form,ep_state,order, N_Qubits, check_reduction=False):
    

    reduced_hamiltonians = get_reduced_hamiltonians(ham,model,fn_form,ep_state,order, N_Qubits, check_reduction=check_reduction)
...
def csvqe_approximations_heuristic(ham, ham_noncon, n_qubits, true_gs, check_reduction=False):
    ...
    if gs_noncon[0]-true_gs < 10**-10:
        ...
        while len(indices)>1 and not exact:
            ...
            for i_subset in itertools.combinations(indices,num_to_remove):
                ...
                approxs_temp = contextual_subspace_approximations(ham,model,fn_form,ep_state, possible_order, n_qubits, check_reduction=False)
        ...
        approxs = contextual_subspace_approximations(ham,model,fn_form,ep_state,deepcopy(order_full), n_qubits, check_reduction=False)
        errors = [a - true_gs for a in approxs]
            
        # print(errors,'\n')
            
        approxs2 = contextual_subspace_approximations(ham,model,fn_form,ep_state,deepcopy(order2), n_qubits, check_reduction=False)
