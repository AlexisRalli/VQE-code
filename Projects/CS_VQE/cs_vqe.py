...
import Unitary_partitioning_LCU_method as UP_LCU 
...
def diagonalize_epistemic_LCU(model,fn_form,ep_state, N_Qubits, N_index, check_reduction=False):
...
def get_reduced_hamiltonians_LCU(ham,model,fn_form,ep_state,order, N_Qubits, N_index, check_reduction=False):

    Rot_LCU, rotations, diagonal_set, vals = diagonalize_epistemic_LCU(model,fn_form,ep_state, N_Qubits, N_index, check_reduction=False)
...
def contextual_subspace_approximations_LCU(ham,model,fn_form,ep_state,order, N_Qubits, N_index, check_reduction=False):
    
    reduced_hamiltonians=get_reduced_hamiltonians_LCU(ham,model,fn_form,ep_state,order, N_Qubits, N_index, check_reduction=check_reduction)
    n_q = len(list(ham.keys())[0])
    ...
    for ham_red in reduced_hamiltonians:
        ...
        if len(list(ham_red.keys())) == 1:
            ...
            ham_red_sparse = qubit_operator_sparse(ham_red_openF, n_qubits=N_Qubits)
            # ham_red_sparse = hamiltonian_to_sparse(ham_red)
...
def csvqe_approximations_heuristic_LCU(ham, ham_noncon, n_qubits, true_gs, N_index, check_reduction=False):
    ...
    if gs_noncon[0]-true_gs < 10**-10:
        ...
        while len(indices)>1 and not exact:
            ...
            for i_subset in itertools.combinations(indices,num_to_remove):
                possible_order = order+list(i_subset)
                current_i = len(possible_order)
                approxs_temp = contextual_subspace_approximations_LCU(ham,model,fn_form,ep_state,possible_order, n_qubits, N_index, check_reduction=check_reduction)
                errors_temp = [a - true_gs for a in approxs_temp]
                if errors_temp[current_i] < 10**-6:
                    exact = True
                improvement = errors_temp[current_i-2]-errors_temp[current_i]
        ...
        approxs = contextual_subspace_approximations_LCU(ham,model,fn_form,ep_state,deepcopy(order_full), n_qubits, N_index, check_reduction=check_reduction)
        errors = [a - true_gs for a in approxs]
            
        # print(errors,'\n')
            
        approxs2 = contextual_subspace_approximations_LCU(ham,model,fn_form,ep_state,deepcopy(order2), n_qubits, N_index, check_reduction=check_reduction)
        errors2 = [a - true_gs for a in approxs2]
